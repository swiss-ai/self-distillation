...
def _build_wandb_tags(config: dict | None) -> list[str]:
    """Extract important tags from the training config for wandb.

    Auto-generated tags include model name, rollout count, algorithm,
    loss mode, batch size, and epochs. Users can add custom tags via
    ``trainer.wandb_tags`` (a list of strings) in the config.
    """
    if config is None:
        return []
    ...
    # -- model name (last component of the path) --
    # model_path = _deep_get(config, "actor_rollout_ref", "model", "path")
    # if model_path:
    #     model_name = str(model_path).rstrip("/").split("/")[-1]
    #     tags.append(f"model:{model_name}")

    # -- rollout count (n) --
    rollout_n = _deep_get(config, "actor_rollout_ref", "rollout", "n")
    if rollout_n is not None:
        tags.append(f"rollouts:{rollout_n}")

    # -- algorithm / advantage estimator --
    adv_estimator = _deep_get(config, "algorithm", "adv_estimator")
    if adv_estimator:
        tags.append(f"algo:{adv_estimator}")

    # -- policy loss mode --
    loss_mode = _deep_get(config, "actor_rollout_ref", "actor", "policy_loss", "loss_mode")
    if loss_mode:
        tags.append(f"loss:{loss_mode}")

    # -- batch size --
    batch_size = _deep_get(config, "data", "train_batch_size")
    if batch_size is not None:
        tags.append(f"bs:{batch_size}")

    # -- epochs --
    epochs = _deep_get(config, "trainer", "total_epochs")
    if epochs is not None:
        tags.append(f"epochs:{epochs}")

    # -- teacher regularization (self-distillation) --
    teacher_reg = _deep_get(
        config, "actor_rollout_ref", "actor", "self_distillation", "teacher_regularization"
    )
    if teacher_reg:
        tags.append(f"teacher_reg:{teacher_reg}")

    # -- user-specified tags --
    user_tags = _deep_get(config, "trainer", "wandb_tags", default=[])
...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            entity = config["trainer"].get("wandb_entity", None) if config else None
            if entity is None:
