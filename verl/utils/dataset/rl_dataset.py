...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        original_data_source = row_dict.get("data_source", None)
        row_dict["extra_info"] = canonicalize_extra_info(
            row_dict.get("extra_info"),
            data_source=original_data_source,
        )
        row_dict["reward_model"] = canonicalize_reward_model(
            row_dict.get("reward_model"),
            scoring_module=row_dict["extra_info"].get("data_source"),
        )
        row_dict["data_source"] = get_data_source(
            row_dict["extra_info"],
            fallback=str(original_data_source or "unknown"),
        )
        row_dict.setdefault("answer", "")
        ...
        if need_tools_kwargs and not tools_kwargs:
            logger.warning(
                ...
                index,
                get_data_source(row_dict["extra_info"], fallback="unknown"),
            )
