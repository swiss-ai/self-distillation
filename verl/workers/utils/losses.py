...
from verl.trainer.ppo.metric_utils import compute_log_ratio_diagnostics
...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    # Log-ratio distribution diagnostics (std, abs_mean, p95, p99, max_abs).
    # Aggregated as MEAN across micro-batches except max_abs, which uses MAX to
    # preserve the true batch-wide maximum.
    log_ratio_diag = compute_log_ratio_diagnostics(
        log_prob=log_prob, old_log_prob=old_log_prob, response_mask=response_mask
    )
    for key, value in log_ratio_diag.items():
        agg = AggregationType.MAX if key.endswith("/max_abs") else AggregationType.MEAN
        metrics[key] = Metric(value=value, aggregation=agg)
