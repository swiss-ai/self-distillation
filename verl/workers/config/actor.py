...
class SelfDistillationConfig(BaseConfig):
    """Configuration for self-distillation training.

    Args:
        full_logit_distillation: Whether to distill full token distributions instead of
            only generated-token log-probs.
        alpha: Distillation interpolation coefficient. `0.0` is forward KL, `1.0`
            is reverse KL, and values in between yield a generalized JSD-like loss.
        success_reward_threshold: Minimum sequence reward used to treat a rollout as
            a successful in-context demonstration.
        teacher_regularization: Teacher update strategy. Supported values are
            ``"ema"`` and ``"trust-region"``.
        teacher_update_rate: EMA update rate or trust-region interpolation weight.
        distillation_topk: Optional top-k truncation for distribution distillation.
        distillation_add_tail: Whether to add an aggregate tail bucket when using top-k.
        max_reprompt_len: Maximum teacher-side prompt length before the generated
            response is appended.
        reprompt_truncation: Truncation policy for reprompt tokenization.
        dont_reprompt_on_self_success: Whether to exclude the sample's own successful
            response from the teacher prompt.
        remove_thinking_from_demonstration: Whether to strip ``<think>`` traces from
            successful demonstrations before reuse.
        is_clip: Optional importance-sampling clip for the distillation loss.
        reprompt_template: Template for the reprompted user message.
        solution_template: Template for embedding a successful demonstration.
        feedback_template: Template for embedding environment feedback.
        include_environment_feedback: Whether to include textual environment feedback.
        environment_feedback_only_without_solution: If true, only use feedback when
            no successful demonstration is available.
        use_dataset_expert_answer: Whether to always use privileged expert answers
            from the dataset as the teacher-side solution context.
        expert_answer_column: Top-level non-tensor batch column to read the privileged
            answer from.
        expert_answer_key: Deprecated legacy field for datasets that used to store the
            privileged answer inside ``extra_info``.
        expert_answer_template: Template used to wrap the privileged answer before it is
            inserted into ``solution_template`` as ``successful_previous_attempt``.
        skip_distillation_on_correct: When True, mask out the distillation loss for
            rollouts that already achieved reward >= ``success_reward_threshold``.
            Only effective when ``use_dataset_expert_answer`` is True.
    """
    ...
    use_dataset_expert_answer: bool = False
    expert_answer_column: str = "answer"
    expert_answer_key: str = "expert_answer"
    expert_answer_template: str = "<answer>{expert_answer}</answer>"
    skip_distillation_on_correct: bool = False
    always_apply_reprompt: bool = False

    def __post_init__(self):
        if not 0.0 <= self.alpha <= 1.0:
            raise ValueError(f"self_distillation.alpha must be in [0, 1], got {self.alpha}")
        if self.teacher_regularization not in {"ema", "trust-region"}:
            raise ValueError(
                "self_distillation.teacher_regularization must be one of ['ema', 'trust-region'], "
                f"got {self.teacher_regularization}"
            )
        if not 0.0 <= self.teacher_update_rate <= 1.0:
            raise ValueError(
                "self_distillation.teacher_update_rate must be in [0, 1], "
                f"got {self.teacher_update_rate}"
            )
        if self.distillation_topk is not None and self.distillation_topk <= 0:
            raise ValueError(
                "self_distillation.distillation_topk must be a positive integer, "
...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        self_distillation (SelfDistillationConfig): Configuration for teacher-side
            prompt construction and SDPO-style distillation settings.
    """
