...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def teacher_rollout_mode(self):
        """Push teacher/EMA weights to the rollout engine (instead of the student)."""
        ...
        if teacher_module is None or teacher_module is self.actor_module_fsdp:
            # No separate teacher — fall back to normal rollout_mode
        ...
        await self.rollout.update_weights(per_tensor_param, base_sync_done=False)
        del params, per_tensor_param
        aggressive_empty_cache(force_sync=True)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if "topk_logps" in outputs:
                tensors["topk_logps"] = outputs["topk_logps"]
...
class AsyncActorRolloutRefWorker(ActorRolloutRefWorker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL, blocking=False)
    async def load_teacher_weights(self):
        """Push teacher/EMA weights to the rollout engine."""
        await self.teacher_rollout_mode()
        return True

    @register(dispatch_mode=Dispatch.ONE_TO_ALL, blocking=False)
    async def restore_actor_weights(self):
        """Restore student/actor weights to the rollout engine."""
        await self.rollout_mode()
        return True
