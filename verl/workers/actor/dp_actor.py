...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _update_teacher(self) -> None:
        ...
        if self_distillation_cfg is None or loss_mode != "sdpo":
            return
        if self_distillation_cfg.teacher_regularization != "ema":
            return
        update_rate = self_distillation_cfg.teacher_update_rate
        if update_rate == 0.0:
            return
    ...
    def _forward_micro_batch(
        ...
        Returns:
            dict[str, torch.Tensor]:
                ...
                if logit distillation is enabled:
                    all_logps / topk_logps / topk_indices
        """
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    if calculate_entropy or compute_all_logps or use_topk:
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy: bool = False) -> dict[str, torch.Tensor]:
        ...
        Args:
            ...
            Optionally, ``data.meta_info["distill_topk"]`` (int) requests top-k log-probs
            and token indices per response position.
        ...
        Returns:
            dict[str, torch.Tensor]: a dict containing keys
                ...
                - ``topk_logps``: (optional) tensor of shape [batch_size, response_length, topk]. torch.float32.
                - ``topk_indices``: (optional) tensor of shape [batch_size, response_length, topk]. torch.int64.
        """
        calculate_sum_pi_squared = self.config.get("calculate_sum_pi_squared", False)
    ...
    def update_policy(self, data: DataProto):
        ...
        if sdpo_enabled and not self_distillation_required_keys.issubset(set(data.batch.keys())):
            missing = sorted(self_distillation_required_keys - set(data.batch.keys()))
            raise ValueError(f"Missing required teacher batch keys: {missing}")
        ...
        if sdpo_enabled:
            select_keys.extend(sorted(self_distillation_required_keys))
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    self_distillation_mask = model_inputs.get("self_distillation_mask") if sdpo_enabled else None
                    if sdpo_enabled and has_multi_modal_inputs:
                        raise ValueError("SDPO currently does not support multi-modal inputs.")
                    ...
                    teacher_regularization = self_distillation_cfg.teacher_regularization if sdpo_enabled else None
                    ...
                    return_all_logps = (
                        bool(sdpo_enabled and self_distillation_cfg.full_logit_distillation)
                        and self_distillation_cfg.distillation_topk is None
                    )
                    distill_topk = (
                        self_distillation_cfg.distillation_topk
                        if sdpo_enabled and self_distillation_cfg.full_logit_distillation
                        else None
                    )
                    ...
                    if sdpo_enabled:
                        ...
                        pg_metrics["self_distillation/empty_target_batch"] = bool(
                            self_distillation_mask is not None and self_distillation_mask.sum().item() == 0
                        )
                    else:
