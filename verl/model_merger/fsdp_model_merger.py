    ...
    def _load_and_merge_state_dicts(
        ...
        for key in sorted(state_dict):
            ...
            if key in param_placements:
                ...
                # Non-DTensor entries are replicated across ranks (e.g. scalar
                # buffers). Concat breaks on 0-dim tensors, so just take rank 0
                # when all shards are identical.
                shards = state_dict[key]
                if all(t.shape == shards[0].shape and torch.equal(t, shards[0]) for t in shards[1:]):
