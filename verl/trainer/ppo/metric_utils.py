...
def tail_compression_ratio(response: str, tail_bytes: int = 2048) -> float:
    """Measure repetition at the tail of a response via zlib compression ratio.

    Takes the last ``tail_bytes`` bytes of the UTF-8 encoded response and returns
    ``len(tail) / len(zlib.compress(tail))``. A value of ~1.0 indicates no
    repetition (incompressible); larger values indicate more repetition.

    Args:
        response: The response text.
        tail_bytes: Number of bytes to take from the end of the response. If the
            response is shorter than this, the full response is used.

    Returns:
        Ratio of raw length to zlib-compressed length of the tail, in bytes.
        Returns 1.0 for empty responses.
    """
    tail = response.encode("utf-8")[-tail_bytes:]
    if not tail:
        return 1.0
    return len(tail) / len(zlib.compress(tail))
...
def compute_log_ratio_diagnostics(
    ...
    """Distribution-shape diagnostics of the actor log ratio ``log π_θ − log π_old``.

    These complement ``actor/ppo_kl`` (the mean) by describing the spread and
    tails of the per-token log ratio. All reductions are taken over valid
    response tokens only.

    Returns:
        Dictionary with:
            - actor/log_ratio/std: standard deviation (unbiased=False)
            - actor/log_ratio/abs_mean: mean absolute log ratio (drift magnitude)
            - actor/log_ratio/p95: 95th percentile of the log ratio
            - actor/log_ratio/p99: 99th percentile of the log ratio
            - actor/log_ratio/max_abs: maximum absolute log ratio
        Empty dict if there are no valid tokens.
    """
    with torch.no_grad():
        log_ratio = (log_prob - old_log_prob).detach()
        mask = response_mask.to(torch.bool)
        valid = log_ratio[mask]
        if valid.numel() == 0:
            return {}
        valid = valid.float()

        std = valid.std(unbiased=False)
        abs_mean = valid.abs().mean()
        max_abs = valid.abs().max()
        quantiles = torch.quantile(valid, torch.tensor([0.95, 0.99], device=valid.device))

    return {
        "actor/log_ratio/std": std.item(),
        "actor/log_ratio/abs_mean": abs_mean.item(),
        "actor/log_ratio/p95": quantiles[0].item(),
        "actor/log_ratio/p99": quantiles[1].item(),
        "actor/log_ratio/max_abs": max_abs.item(),
    }


def compute_response_repetition_metrics(
    batch: DataProto,
    tokenizer,
    tail_bytes: int = 2048,
) -> dict[str, Any]:
    """Compute the tail compression ratio repetition metric over a batch.

    For each response in the batch, the response tokens are decoded to text
    (skipping special tokens), and :func:`tail_compression_ratio` is applied.

    Args:
        batch: A DataProto containing response token ids under ``responses`` and
            the corresponding ``response_mask``.
        tokenizer: Tokenizer used to decode response tokens to text.
        tail_bytes: Number of bytes to take from the end of each response.

    Returns:
        A dictionary with mean/max/min of the tail compression ratio across the
        batch (non-aborted samples only).
    """
    ...
    for i in range(responses.shape[0]):
        valid_ids = responses[i][response_mask[i]]
        if valid_ids.numel() == 0:
            continue
        text = tokenizer.decode(valid_ids, skip_special_tokens=True)
        ratios.append(tail_compression_ratio(text, tail_bytes=tail_bytes))

    if not ratios:
        return {}

    return {
        "response/tail_compression_ratio/mean": float(np.mean(ratios)),
        "response/tail_compression_ratio/max": float(np.max(ratios)),
        "response/tail_compression_ratio/min": float(np.min(ratios)),
    }


# Matches phrasings like "expert solution", "reference solution", "correct solution"
# (case-insensitive, tolerant of multiple whitespace characters between words).
_LEAKAGE_PATTERN = re.compile(r"\b(expert|reference|correct|original)\s+solution\b", re.IGNORECASE)


def compute_response_leakage_metrics(batch: DataProto, tokenizer) -> dict[str, Any]:
    """Detect references to privileged information in model responses.

    Flags responses that mention "expert solution", "reference solution", or
    "correct solution" (case-insensitive) — language that typically indicates the
    model is leaking knowledge of privileged teacher/oracle content into its own
    outputs.
    ...
    Returns:
        A dictionary with:
            - response/leakage/any_match_frac: fraction of responses containing
              at least one match.
            - response/leakage/match_count_mean: mean number of matches per
              response.
        Empty dict if there are no non-aborted responses.
    """
    responses = batch.batch["responses"]
    response_mask = batch.batch["response_mask"].bool()
    ...
    for i in range(responses.shape[0]):
        ...
        if valid_ids.numel() == 0:
            continue
        text = tokenizer.decode(valid_ids, skip_special_tokens=True)
        match_counts.append(len(_LEAKAGE_PATTERN.findall(text)))

    if not match_counts:
        return {}

    match_counts_arr = np.asarray(match_counts, dtype=np.float32)
    return {
        "response/leakage/any_match_frac": float((match_counts_arr > 0).mean()),
        "response/leakage/match_count_mean": float(match_counts_arr.mean()),
    }
