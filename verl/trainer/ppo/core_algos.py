...
def compute_self_distillation_loss(
    ...
    This implements either sequence-token reverse KL on sampled tokens or
    distribution distillation using full logits / top-k logits.

    Args:
        student_log_probs (torch.Tensor):
            Student log-probabilities on sampled response tokens, shape
            (batch_size, response_length).
        teacher_log_probs (torch.Tensor):
            Teacher log-probabilities on the same sampled response tokens, shape
            (batch_size, response_length).
        response_mask (torch.Tensor):
            Binary mask for valid response tokens, shape
            (batch_size, response_length).
        self_distillation_config (Any):
            Self-distillation config object controlling whether to use sampled-token
            distillation or full-logit distillation, the divergence direction, top-k
            truncation, and importance-weight clipping.
        old_log_probs (Optional[torch.Tensor]):
            Log-probabilities from the old policy used to compute SDPO importance
            ratios when `self_distillation_config.is_clip` is enabled, shape
            (batch_size, response_length).
        student_all_log_probs (Optional[torch.Tensor]):
            Student full-vocabulary log-probabilities used when
            `full_logit_distillation=True` and `distillation_topk is None`, shape
            (batch_size, response_length, vocab_size).
        teacher_all_log_probs (Optional[torch.Tensor]):
            Teacher full-vocabulary log-probabilities used when
            `full_logit_distillation=True` and `distillation_topk is None`, shape
            (batch_size, response_length, vocab_size).
        student_topk_log_probs (Optional[torch.Tensor]):
            Student top-k log-probabilities used when
            `full_logit_distillation=True` and `distillation_topk` is set, shape
            (batch_size, response_length, topk).
        teacher_topk_log_probs (Optional[torch.Tensor]):
            Teacher top-k log-probabilities used when
            `full_logit_distillation=True` and `distillation_topk` is set, shape
            (batch_size, response_length, topk).
        self_distillation_mask (Optional[torch.Tensor]):
            Optional per-sequence mask indicating which samples should contribute to
            the distillation objective, shape (batch_size,).
        loss_agg_mode (str):
            Aggregation mode passed to `agg_loss` for reducing per-token losses into
            a scalar.
        rollout_is_weights (Optional[torch.Tensor]):
            Optional rollout importance-sampling weights applied multiplicatively to
            the per-token distillation loss, shape (batch_size, response_length).
        global_batch_info (Optional[dict[str, Any]]):
            Optional metadata forwarded to `agg_loss`, such as `dp_size`,
            `batch_num_tokens`, `global_batch_size`, and `loss_scale_factor`.

    Returns:
        tuple[torch.Tensor, dict[str, Any]]:
            A tuple containing the scalar aggregated loss, shape (), and a metrics
            dictionary with SDPO diagnostics.
    """
    ...
    with torch.no_grad():
        teacher_vs_student = (teacher_log_probs - student_log_probs).detach()
        if loss_mask.sum() > 0:
            metrics["self_distillation/log_ratio_mean"] = verl_F.masked_mean(
                teacher_vs_student, loss_mask
            ).item()
            metrics["self_distillation/log_ratio_abs_mean"] = verl_F.masked_mean(
                teacher_vs_student.abs(), loss_mask
            ).item()
        else:
            metrics["self_distillation/log_ratio_mean"] = 0.0
            metrics["self_distillation/log_ratio_abs_mean"] = 0.0
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        if self_distillation_config.alpha != 1.0:
            raise ValueError("Only reverse KL is supported when full_logit_distillation is disabled.")
    ...
    if is_clip is not None:
        ...
        metrics["self_distillation/is_ratio_mean"] = verl_F.masked_mean(ratio, loss_mask).detach().item()

    if rollout_is_weights is not None:
        per_token_loss = per_token_loss * rollout_is_weights

    metrics["self_distillation/active_token_fraction"] = (
        loss_mask.sum() / response_mask.sum().clamp(min=1.0)
    ).detach().item()
