...
def need_self_distillation_teacher(
    config: DictConfig,
) -> bool:
    """Given the config, do we need a separate teacher model for SDPO."""
    actor_config = config.actor_rollout_ref.actor
    loss_mode = actor_config.policy_loss.get("loss_mode", "vanilla")
    return loss_mode == "sdpo"
