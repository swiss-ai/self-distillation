...
from verl.trainer.ppo.utils import need_critic, need_reference_policy, need_self_distillation_teacher
...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if use_legacy_worker_impl == "disable":
            ...
            if need_self_distillation_teacher(config) and ref_in_actor:
                raise ValueError(
                    "Teacher-based objectives require a separate teacher model and are not supported with ref_in_actor LoRA."
                )
            ...
            if (need_reference_policy(config) or need_self_distillation_teacher(config)) and not ref_in_actor:
        ...
        if need_reference_policy(config) or need_self_distillation_teacher(config):
            role = Role.ActorRolloutRef
        else:
            role = Role.ActorRollout
    ...
    def add_ref_policy_worker(self, config, ref_policy_cls):
        ...
        use_legacy_worker_impl = config.trainer.get("use_legacy_worker_impl", "auto")
        if use_legacy_worker_impl == "disable" or Role.ActorRolloutRef in self.role_worker_mapping:
            return
    ...
    def run(self, config):
        ...
        validate_config(
            config=config,
            use_reference_policy=need_reference_policy(config) or need_self_distillation_teacher(config),
        ...
        skip_dataset_validation = config.trainer.get("skip_dataset_validation", False)
        if skip_dataset_validation or not config.data.val_files:
            val_dataset = None
        else:
